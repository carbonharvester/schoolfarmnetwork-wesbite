...
const MotionIndicator=()=> <span className="motion-track" aria-hidden="true"><span className="motion-thumb"/></span>;
export default function App(){
 ...
 return <div ref={scope} id="top" className="direction-photo"><a className="skip-link" href="#main">Skip to content</a>
  ...
  <footer className="contact" id="contact"><div className="contact-top"><h2>Let's grow<br/>something<br/><em>that lasts.</em></h2><div className="contact-copy"><p>{content.contact}</p><a className="pill pill-lime" href={mailto}>Start a conversation <Arrow/></a><div className="email-row"><a href={mailto}>{content.email}</a><button onClick={copyEmail} aria-label="Copy email address" hidden={!ready}><span aria-hidden="true">{notice==='Email address copied.'?'✓':'⧉'}</span>{notice==='Email address copied.'?'Copied':'Copy email'}</button></div><p className="live-notice" role="status" aria-live="polite">{notice}</p></div></div><div className="footer-wordmark" aria-hidden="true"><span>School Farm</span><span>Network<sup>●</sup></span></div><p className="image-disclosure">Imagery is AI-generated to illustrate our vision.</p><div className="footer-bottom"><p>© {new Date().getFullYear()} School Farm Network</p><button className="footer-motion" onClick={toggleMotion} aria-pressed={motion} hidden={!ready}><MotionIndicator/><span>Motion {motion?'on':'off'}</span></button><a className="back-top" href="#top">Back to top <Arrow direction="up"/></a></div></footer>
 </div>
}
