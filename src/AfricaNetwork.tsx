...
export default function AfricaNetwork(){
 ...
 useEffect(()=>{
  ...
  let width=500,height=500,visible=false,frame=0,last=0,clock=0,phase=0;const state={progress:0};
  ...
  const resize=()=>{const b=c.getBoundingClientRect();width=b.width;height=b.height;const dpr=Math.min(devicePixelRatio||1,2);c.width=width*dpr;c.height=height*dpr;g.setTransform(dpr,0,0,dpr,0,0);draw(0)};
  ...
  const ctx=gsap.context(()=>{gsap.to(state,{progress:1,ease:'none',scrollTrigger:{trigger:root.current!.querySelector('.roots-stage'),start:()=>`top ${document.querySelector('.header')?.getBoundingClientRect().height||72}px`,end:()=>'+='+innerHeight*(innerWidth<768?4.5:3.8),pin:true,pinType:'transform',scrub:.7,invalidateOnRefresh:true},onUpdate:()=>{const n=state.progress<.25?0:state.progress<.5?1:state.progress<.76?2:3;if(n!==phase){phase=n;setActive(n)}if(!visible)draw(0)}})},root);
